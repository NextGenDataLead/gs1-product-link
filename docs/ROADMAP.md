...
#75 turned out to be a sample rather than the finding. It named four media fixes that had never
crossed from `lib/wp_client.py` to the WordPress MCP; auditing the *rest* of that client
immediately after merging #84 found **five more**, all older, all in code it does implement — and
[#85](https://github.com/NextGenDataLead/gs1-product-link/pull/85) ports those. Three of the five
are one commit, `e9898a5` "language-scoped lookups, ?lang= on create, ACF as its own call": the
commit where the Python client learned how multilingual publishing works. The pilot site is nl/fr,
so the TS client was not lagging, it predated the project understanding the problem.

**No CI job can see this drift**, and none can be written to: nothing on the publish path imports
the MCPs, so both implementations stay green while they disagree. Both rounds were found by reading
the two files against each other, which is why `mcps/wordpress/README.md` now carries an audit date,
the list of what is absent by scope, and the two commands that repeat the check. The open question
that leaves — whether a second implementation with no consumer earns its maintenance at all — is
recorded there rather than settled here.
...
| [#85](https://github.com/NextGenDataLead/gs1-product-link/pull/85) | **Auditing the rest of that client found five more drifted fixes, older than #75's four.** Lookups were not language-scoped — and unscoped, a slug lookup answers for the default language only, so the fr row would find the nl page, the GTIN would match (both languages share it, so E8 cannot catch it) and **the nl page would be overwritten with French**, row reporting ok. Creates carried no `?lang=`, so the fr page would collide with its nl sibling, take a `-2` slug, and **every French QR would 404**. ACF had no input at all, though writing it on a `?lang=` create silently drops it. `findByMetaGtin` trusted `meta_key`/`meta_value` to have filtered, which core does not honour without an enabler — so it adopted an arbitrary page and turned every create into a bogus slug collision. And WPML was probed at a path WPML does not register, so detection could never return `wpml`. Each fix mutation-tested; M5 exposed that the year-old detection tests never asserted *which* URL was probed, which is how the wrong path stayed green. |
...
| [#95](https://github.com/NextGenDataLead/gs1-product-link/pull/95) | **The video-mapping hints had been scoring on the product name alone, and no test noticed.** `_candidate_fields` reads `marketing_name` and `logistics_name` out of flat `extras`; #93 moved every `localised: true` extra into `extras_localised`, so the loop returned nothing from then on. That matters because the filenames are English and the feed's English is in those two extras — attr 3301 reads "huisdierspeelgoed" where the French `marketing_name` reads "Noviplast Pet Buddy". **Nothing failed, and the two tests that should have are the finding**: the unit test built a *flat* extra the parser no longer produces, and the draft test asserted `"hint" in out.lower()`, which matches the printed header whether or not a single hint ranked. Both are now written so that neither can pass on a dead loop. Measured against the 48 client-signed-off rows, the true GTIN went from #1 for 23 to **#1 for 36**, sixteen rows moving and none backwards; for the 118 still unset, the best hint clears 0.90 for 9 files where it did for 2. Found by #94's mutation **M9**, which was expected green and recorded as such rather than waved through — the loop was dead whatever keys were in it. Eight mutations here, one recorded green. |
| [#94](https://github.com/NextGenDataLead/gs1-product-link/pull/94) | **One GDSN attribute was declared as two fields, and every consumer paid for it.** Attr 3301 was mapped as `product_name` *and* declared again as `gdsn_extras.functional_name` — byte-identical for all 127 products in both languages, necessarily so, being the same sheet and attribute. The duplication surfaced wherever the value was consumed: §4 of the report asked for one MyGS1 paste as two jobs, the coverage matrix carried two identical columns, both client templates printed the product name under itself, and `_gather_inputs` read the extra with the mapped field as a fallback for a value that could not differ. The previous release had *deduplicated* §4 rather than removed the cause; that dedupe is gone, because it would have absorbed the next duplicate instead of surfacing it. `lib.config` now refuses a config where two sources read the same `(sheet, attribute)` — at load, not in a test over `clients.example.yml`, because the duplicate lived in the gitignored `clients.yml` no test in git can see. Removing the extra also closed a hole the duplicate had been hiding: a unit with no name in its own language **and** no extra was handing the producer nothing at all. Eight mutations run; two were expected green and recorded as such. |
| [#96](https://github.com/NextGenDataLead/gs1-product-link/pull/96) | **A product made of two materials published as one, and the flag that would have fixed it was a no-op where it was needed.** `BrickGPCCommercialData` spreads materials across `Material[0..2]`, all three labelled `Material (4.012)`; the parser read slot 0 and discarded the rest. `multivalue` exists for exactly this and had been solving it for attr 1067 for months — but it was honoured on **one of four** resolution paths, the localised *mapped field*. `material` is a language-agnostic *extra*, so setting the flag on it did nothing; a localised *extra* was truncated the same way. The docstring said "Ignored for non-localised sources" and that sentence had been read as a design decision rather than a gap. `pick_scalar_all` is now the twin of `pick_localised_all` and one `_scalar_picker` makes the choice for both language-agnostic resolvers, so the flag cannot mean one thing on one path and nothing on the others again; `pick_scalar` keeps its exact behaviour, so nothing that did not ask for `multivalue` moves. **The separator differs by kind on purpose** — a newline for localised sources, because the generator splits 1067 back into ranked USPs on it; `", "` for language-agnostic ones, because that value renders verbatim in Technische details, where a newline collapses to a space and `kunststof metaal` reads as one fused material. **The placeholder guard had to follow**: `_is_placeholder` tested the whole string, so a joined `"kunststof, zzzanders"` would have put the datapool's own "no value" marker on the live page *and* asked the operator to paste it into MyGS1 — the same shape as #93's `zzzanders` finding, the guard rail standing beside the door rather than in it. Measured against the real export in both directions: **exactly seven of 127 products change and `material` is the only field that differs anywhere**; no joined value contains a placeholder slot. Ten mutations, eight killed. One survivor changed the design — the guard's early return protected nothing but *empty* slots, since `sep.join(s.split(sep))` is an identity, so it made the gap in `a, , b` survive while the one in `a, , zzzanders` vanished; one rule for slots removed the branch. A second was expected green and became killable: nothing pinned the blank-slot-0 contract `pick_scalar` documents, so `pick_scalar_all()[0]` looked like a free simplification. Two recorded green, both verified against all 127 products rather than argued. |
| [#100](https://github.com/NextGenDataLead/gs1-product-link/pull/100) | **The data-quality report marked its mandatory columns in bold, and markdown table headers are bold already — so the mark rendered as nothing.** §0's coverage matrix carries the one distinction that decides what an operator does with a gap: a blank in a mandatory column holds the whole SKU (E23, E24 for video), a blank in an optional one only thins the page. That was expressed as `**…**` around the header text, which renders identically to the header without it, and the legend said *"Bold columns are mandatory"* while pointing at nothing visible; fourteen columns all looked the same. Markdown has no row above the header to put a group label in, so the label goes inside the cell — `MANDATORY<br>product·3301`, `optional<br>material`. **The video column moved in with them**: a GTIN without a client-confirmed video is held out of publishing entirely, but the column rendered last, after the optional block and beside `score`, leaving a third group at the far end that made "mandatory first" untrue of the row as a whole. `_columns` is now the only thing that orders the matrix, video included — which is also what keeps the new pin honest, since a renderer that re-partitioned by `required` itself would make a "one crossing" test unfailable. The pin is a **regression pin, not a fix**: the order was already right, and it is worth having because the split derives from `required` / `required_group` in the **gitignored** `clients.yml`, so a field flipping its flag re-groups the header with no code change and nothing in git can see the config that did it — #94's reasoning again. Measured on the real 37-product report: **every cell and every score identical under the same column names**, same row order, rest of the report byte-identical; only the header, the legend and the video cell's position move. The `<br>` is checked against the renderer the operator shell actually uses, because asserting a source string is precisely what missed this defect — and that test was checked for being decorative and is not: under `safe_mode="escape"` the label reaches the screen as literal text, which no source-level assertion can see. Test cell lookups counted backwards from the end of the row and broke on the reorder for the second time; they look the column up by header name now. Eight mutations, eight killed. |
| [#125](https://github.com/NextGenDataLead/gs1-product-link/pull/125) | **The Data screen was the same shape once more, and this time the fix was to refuse it.** The ask was to give the scope list a run-status column: mark deselected rows there, have the run read the file filtered on `status != "deselected"`, then fill the blanks with pass/fail. **That file used to have status columns and they were removed because they failed silently in both directions** — a client whose file said `no` got the opposite of the word, because `"no"` is non-blank; a wrong "on website" emptied the plan and the run reported success having published nothing, a wrong "in GS1" pointed the pipeline at a GTIN with no resolver record, and neither raised anything. The column would also have been **one field answering two questions** — *should this run?* before, *what happened?* after, with re-run semantics undefined — and it could not hold the data it existed for: `RunOutcome` is keyed `(gtin, language)` and this client publishes nl+fr, so one status column on a one-row-per-SKU sheet cannot say "nl ok, fr failed", which has actually happened here. So: one control file where it already is, selection stays a `keeping()` prune, the upload is archived on arrival so **Restore** can undo a deselection, and the run status is a **separate per-run report** written after, joining the archive with the run log. Two status columns in it rather than one, for the same reason the single column was refused. Also fixed, and the reason the screen could not be left alone: the shell read the control file with openpyxl and a header fixed at row 1, while a run reads it namespace-agnostically and scans every sheet — so a real **Strict OOXML** file with a title row above the table loaded in a run and failed on screen. |
...
| [#126](https://github.com/NextGenDataLead/gs1-product-link/pull/126) | **The example client had no export, so every rehearsal of the Data screen used the real one.** `democlient` is defined in `clients.example.yml` and had only a stand-in `products.xlsx` whose `products.json` was seeded directly — it could never clear the both-files-required gate, so the live client was the only route to a populated screen, and uploading through the picker replaces the control file *in place*: 38 rows to 33 over three rounds during #125, the third unexplained. `python -m scripts.make_demo_export` now writes a 24-sheet GDSN workbook and a scope list to the paths the config declares, and **refuses to overwrite an existing file without `--force`**, naming every clash rather than the first — the one thing it could destroy is a client's real export. The fixture is faithful rather than sufficient: the same GTIN once per market with **which market holds a value varying by product**, two markets disagreeing about one French name, a language in one market only, a language absent everywhere, repeated groups in both the localised (1067) and language-agnostic (Material) spellings, a case-level row and an empty one. Nine of twelve products publish and three are held, one per mandatory rule. Barcodes sit on GS1 prefix `029` — restricted distribution, never issued — so none can name a real product. `lib/gdsn_layout.py` is the writing mirror of `lib.gdsn._parse_columns` and is round-tripped through it, and the fixture test reads `clients.example.yml` rather than restating it. Also corrected: the screenshot recipe still described the stand-in, and did not say that `DEFAULT_CLIENTS_PATH` is repo-rooted — so a scratch cwd beside the real checkout still reads the real `clients.yml`. |
