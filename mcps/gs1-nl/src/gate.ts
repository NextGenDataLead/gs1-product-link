 ...
 * could call `gs1_digital_link_upsert` and the record was written — **permanently**. The v2 API has
 * no DELETE; retraction only clears links and disables the record. `CLAUDE.md` says publishing goes
 * through `flow-orchestrator` precisely because "calling `scripts/run_execute.py` directly bypasses
 * every one of them" — an ungated tool call is the same bypass wearing a different hat, against the
 * one surface in this project where a mistake cannot be undone.
 *
  ...
  constructor(
    ...
    super(
      `cannot reach an operator for the ${gateId} gate, so nothing was written. This tool writes ` +
        `permanent, undeletable GS1 records and will not do so unattended. Connect a client that ` +
        `supports MCP elicitation, or publish through the flow-orchestrator skill, which carries ` +
        `every gate. ` +
        `(${String(cause)})`,
    );
        ...
        requestedSchema: {
          ...
          properties: {
            [CONFIRM_KEY]: {
              type: "boolean",
              title: gate.title,
              description: `Yes writes to GS1. No stops the run and writes nothing.`,
            },
          },
